    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if route == "/health":
            ...
            self._send_json(404, {"error": "not found",
                                  "routes": ["/health", "/analyze?symbol=SYMBOL"]}, origin)
...
def build_server(
    ...
    """Construct (but do not start) the bridge server. `analyze_fn` is injectable
    so tests can drive the SSE framing without touching the network or quant core.
    Pass `port=0` for an ephemeral port (read it back from `server_address`)."""
...
def run_server(
    ...
    echo("  GET /health   GET /analyze?symbol=RELIANCE   (Server-Sent Events)")
